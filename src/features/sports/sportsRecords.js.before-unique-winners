...
function isWinner(entry, candidates, config) {
  const entryValue = numericValue(entry, config)
  if (entryValue === null || candidates.length < 2) return false

  const values = candidates
    .map(candidate => numericValue(candidate, config))
    .filter(value => value !== null)

  if (values.length < 2) return false

  const best = config.direction === 'min'
    ? Math.min(...values)
    : Math.max(...values)
...
export function recordsForEntry(entry, allEntries = []) {
  ...
  const exactSport = allEntries
    .filter(candidate =>
      ...
      validDate(candidate) &&
      validDate(candidate) <= entryDate
    )
    .sort((a, b) => validDate(a) - validDate(b))

  const earlierEntries = exactSport.filter(candidate =>
    candidate.id !== entry.id &&
    validDate(candidate) < entryDate
  )

  // First entry is a benchmark rather than a personal best.
  if (earlierEntries.length === 0) return []

  const year = entryDate.getFullYear()
  const yearStart = new Date(year, 0, 1)
  const yearEnd = new Date(year, 11, 31, 23, 59, 59, 999)

  const calendarYearEntries = exactSport.filter(candidate => {
    const date = validDate(candidate)
    return date >= yearStart && date <= yearEnd
  })

  const earlierYearEntries = calendarYearEntries.filter(candidate =>
    candidate.id !== entry.id &&
    validDate(candidate) < entryDate
  )
  ...
  for (const config of configForSport(entry.category)) {
    const everCandidates = [...earlierEntries, entry]
    const everWinner = isWinner(entry, everCandidates, config)
    ...
    const yearCandidates = [...earlierYearEntries, entry]
    const yearWinner =
      earlierYearEntries.length > 0 &&
      isWinner(entry, yearCandidates, config)
    ...
    if (yearWinner) {
      achievements.push({
        ...
        level: 'year',
        icon: '🥇',
        label: `${config.label} this year · ${year}`,
        value: numericValue(entry, config),
        year,
